...
public class DBqueries {
    ...
    public static List<CategoryModal> categoryModalList = new ArrayList<>(); //horizontal menus as parent list

    public static List<List<HomePageModel>> lists = new ArrayList<>(); // child list of parent menu list for further operations eg. mobile, furniture
                            ...
                            if ((long) documentSnapshot.get("view_type") == 0) {
                                ...
                                for (long x = 1; x < no_of_banners + 1; x++) {
                                    sliderModalList.add(new SliderModal(documentSnapshot.get("banner_" + x)
                                            .toString(), documentSnapshot.get("banner_" + x + "_background").toString()));
                                }
                    ...
                    for (long x = 0; x < (long) task.getResult().get("list_size"); x++) {
                        ...
                        if (DBqueries.wishList.contains(ProductDetailsActivity.productID)) {
                            ...
                            if (ProductDetailsActivity.addToWishlistBtn != null) {
                                ProductDetailsActivity.addToWishlistBtn.setSupportImageTintList(context.getResources().getColorStateList(R.color.primary));
                            }
                        } else {
                            if (ProductDetailsActivity.addToWishlistBtn != null) {
                        ...
                        if (loadProductData) {
                            ...
                            firebaseFirestore.collection("PRODUCTS").document(product_ID)
                                ...
                                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                                    if (task.isSuccessful()) {
                                        wishlistModelList.add(new WishlistModel(product_ID,task.getResult().get("product_image_1").toString(),
